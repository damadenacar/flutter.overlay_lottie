...
mixin OverlayLottie<T extends StatefulWidget> on TickerProviderStateMixin<T> {
  ...
  Future<bool> showLottieOverlayDuring(Duration period, { String? animationUrl, VoidCallback? onHide }) {
    return showLottieOverlayWhile(
      () async => await Future.delayed(period), 
  ...
  /// 
  /// It returns true if the callback is started; false otherwise (i.e. if the animation is already shown)
  Future<bool> showLottieOverlayWhile(Function callback, {Duration? timeout, Function? onTimeout, String? animationUrl, VoidCallback? onHide}) async {
    if (_visible) {
      return false;
    }

    _showLottieOverlay(repetitions: double.maxFinite.toInt(), animationUrl: animationUrl, onHide: onHide);
    if (timeout == null) {
      await callback();
    } else {
  ...
  Future<bool> showLottieOverlay({bool? force, int? repetitions, String? animationUrl, VoidCallback? onHide}) async {
  ...
  bool _showLottieOverlay({String? animationUrl, bool force = false, int? repetitions, VoidCallback? onHide}) {
    if (_visible && ! force) {
      return false;
    }
  ...
  Widget buildWithLottieOverlay(BuildContext context, { 
    ...
    if (_visible && (animationUrl != null)) {
      ...
      if (blurContent) {
        content = ImageFiltered(imageFilter: ImageFilter.blur(sigmaX: 1, sigmaY: 1), child: content,);
      }
